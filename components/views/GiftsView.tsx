      ...
      <div className="p-5 pb-32">
        ...
        <div className="mb-8">
            <div className="flex justify-between items-start mb-4">
                <h2 className="text-5xl font-black uppercase leading-[0.8] tracking-tighter text-black dark:text-white">
                    Gift<br/><span className="text-[#A3E635]">Lab</span>
                </h2>
                <button 
                    onClick={() => setShowAdd(!showAdd)}
                    className={`w-12 h-12 border-2 border-black dark:border-white flex items-center justify-center shadow-[3px_3px_0px_0px_#000] active:shadow-none active:translate-y-[2px] transition-all ${showAdd ? 'bg-red-500' : 'bg-[#A3E635]'}`}
                >
        ...
        {showAdd && (
          <div className="mb-10 animate-fade-in-up">
            <div className="bg-white dark:bg-[#111] border-4 border-black dark:border-white p-5 shadow-[8px_8px_0px_0px_#A3E635] space-y-4">
            ...
            {gifts.length === 0 ? (
                ...
                gifts.map((gift) => (
                    <div key={gift.id} className="animate-pop">
                        <div className="bg-white dark:bg-[#111] border-2 border-black dark:border-white p-4 shadow-[4px_4px_0px_0px_#000] dark:shadow-[4px_4px_0px_0px_#A3E635] group">
                            <div className="flex justify-between items-start mb-3">
                                <div>
                                    <div className="inline-block bg-[#7C3AED] text-white text-[8px] font-black px-2 py-0.5 uppercase mb-1">
                                        {gift.category}
                                    </div>
                                    <h4 className="text-2xl font-black uppercase leading-tight dark:text-white group-hover:text-[#7C3AED] transition-colors">
                                        {gift.name}
                                    </h4>
                                </div>
                                <button 
                            ...
                            <div className="flex flex-wrap gap-4 text-xs mb-4">
                                <div className="flex items-center gap-1.5 font-bold dark:text-white">
                                    <User size={14} className="text-[#A3E635]" />
                                ...
                                {gift.link && (
                                    <a 
                                        ...
                                        target="_blank" 
                                        rel="noreferrer"
                                        className="flex items-center gap-1.5 font-bold text-[#7C3AED] dark:text-[#A3E635] hover:underline"
                                    >
                            ...
                            {gift.description && (
                                <div className="bg-gray-50 dark:bg-black p-3 border-l-4 border-[#A3E635] italic text-sm font-medium dark:text-gray-300">
                                    {gift.description}
                                </div>
                            )}
                        </div>
